...
<div class="ztop">
	...
	<div>
	<a href="javascript:void(0)" onclick="mle.fun.toUrl('my_kickback_bill.php', false);" class="button_css_1 activity">收入账单</a><a href="javascript:void(0)" onclick="mle.fun.toUrl('my_integral_log.php', false);" class="button_css_1">积分账单</a>
	</div>
</div>
<div class="sea">
    	...
    	<li>
        	<table width="100%">
            	<tr>
                	<td>
                    	<h3>
                    	{:if $bill_value['kickback_type']==0:}
                    		【{:$bill_value['name']:}】贡献{:if $bill_value['level']>0:}{:fun::num_toch($bill_value['level'], false):}级{:/if:}分销佣金
                    	{:elseif $bill_value['kickback_type']==1:}
                 	   		【{:$bill_value['name']:}】贡献{:if $bill_value['level']>0:}{:fun::num_toch($bill_value['level'], false):}级{:/if:}代理商佣金
                    	{:elseif $bill_value['kickback_type']==2:}
                 	   		【{:$bill_value['name']:}】首次关注，获得厂家补贴
                    	{:elseif $bill_value['kickback_type']==3:}
                 	   		{:$bill_value['content']:}
                    	{:elseif $bill_value['kickback_type']==4:}
                 	   		{:$bill_value['content']:}
                    	{:/if:}
                    	</h3>
                        <p>
	                    	...
	                    	{:elseif $bill_value['kickback_type']==1:}
	                 	   		订单号：{:$bill_value['oid']:}&nbsp;&nbsp;{:date('Y-m-d H:i:s', $bill_value['addtime']):}
	                    	{:elseif $bill_value['kickback_type']==2:}
	                 	   		厂家补贴&nbsp;&nbsp;{:date('Y-m-d H:i:s', $bill_value['addtime']):}
	                    	{:elseif $bill_value['kickback_type']==3:}
	                 	   		悬赏令&nbsp;&nbsp;{:date('Y-m-d H:i:s', $bill_value['addtime']):}
	                    	{:elseif $bill_value['kickback_type']==4:}
	                 	   		积分分红&nbsp;&nbsp;{:date('Y-m-d H:i:s', $bill_value['addtime']):}
	                    	{:/if:}
                        </p>
                    </td>
                    <td align="right"><span style="font-size:16px; color:{:if $bill_value['money']>0:}#009900{:else:}#CC3333{:/if:}">{:if $bill_value['money']>0:}+{:/if:}{:$bill_value['money']:}</span></td>
                </tr>
            </table>
        </li>
    {:foreachelse:}
