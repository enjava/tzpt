...
<div class="dd">
	...
	{:foreach $mle['re_address_data'] as $adddress_val name=foo:}
    	<li>
            <div>
                ...
                <p>地址：{:$adddress_val['address']:}</p>
                <p>电话：{:$adddress_val['phone']:}</p>
                <p>
                	<a href="{:$var_x['path']['mleroot_url']:}/mobile/add_address.php?address_id={:$adddress_val['id']:}&goto_type=old_address">【编辑】</a>
                	<a onclick="if(confirm('你确定删除该地址吗？')==false) return; mle.fun.toUrl('?action=del_address&address_id={:$adddress_val['id']:}', false);" href="javascript:void(0);">【删除】</a>
                </p>
            </div>
    ...
    <a href="{:$var_x['path']['mleroot_url']:}/mobile/add_address.php?goto_type=old_address" class="ps">新增一个收货地址</a>
</div>
</form>
